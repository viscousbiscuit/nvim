...
local curl_execute_func = require("plugins/scratch/curl_execute_func")
local hurl_execute_func = require("plugins/scratch/curl_execute_func")
local php_execute_func = require("plugins/scratch/php_execute_func")
...
local langs = {
	curl = {
		name = "curl",
		ext = "sh",
		executeFunction = curl_execute_func,
	},
	hurl = {
		name = "hurl",
		ext = "hurl",
		executeFunction = hurl_execute_func,
	},
	...
	php = {
		name = "php",
		ext = "php",
        executeFunction = php_execute_func,
	},
	sql = {
		name = "sql",
		ext = "sql",
...
local executeScript = function()
	...
	if #filenameParts > 1 then
		...
		if matchedLang ~= nil then
            ...
            if matchedLang.executeFunction ~= nil then
                matchedLang.executeFunction()
            end
		end
	end
end
